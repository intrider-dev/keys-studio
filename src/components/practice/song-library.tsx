  ...
  async function show(value: boolean) {
    ...
    if (value) {
      try {
        ...
        const local = response.ok ? await response.json().catch(() => null) : null;
        setCatalog(Array.isArray(local) ? local : await fetch("/songs/catalog.example.json").then(r => r.json()));
      } catch {
    ...
    <Dialog open={open} onOpenChange={(value) => void show(value)}>
      ...
      <DialogContent className="flex max-h-[90svh] flex-col gap-0 overflow-hidden p-0 sm:max-w-2xl">
        <DialogHeader className="shrink-0 border-b px-6 py-5 pr-12">
          <DialogTitle>Библиотека произведений</DialogTitle>
          <DialogDescription>
            Импортируйте MIDI или откройте подборку. Темп, партия, тембр и
            оформление сохраняются для каждой песни.
          </DialogDescription>
        </DialogHeader>
        <div
        ...
        >
          <div className="rounded-xl border border-dashed bg-muted/30 p-5 text-center">
            ...
            <p className="mb-3 mt-1 text-xs text-muted-foreground">
              .mid, .midi или .pianopack, можно несколько сразу
            </p>
            ...
            <input
              ...
              className="hidden"
              aria-label="Импорт произведений"
              onChange={(e) => {
          ...
          <div className="flex gap-2">
            <Input
              aria-label="Ссылка на MIDI"
              placeholder="Прямая ссылка на .mid"
              value={url}
          ...
          <p className="text-xs text-muted-foreground">
            Если сайт запрещает загрузку по ссылке, скачайте файл и перетащите
            сюда. Из MuseScore экспортируйте MIDI. PDF нужно сначала распознать
            или преобразовать.
          </p>
          {busy && (
            <p role="status" className="flex items-center gap-2 text-sm">
              <LoaderCircle className="size-4 animate-spin" />
              Добавляю произведение…
            </p>
          )}
          {error && (
          ...
          <div className="relative">
            ...
            <Input
              className="pl-9"
              placeholder="Найти произведение"
          ...
          <div className="space-y-2">
            <h3 className="text-sm font-medium">Моя библиотека</h3>
            {entries
              ...
              .map((entry) => (
                ...
                >
                  ...
                  >
                    ...
                    <span className="truncate">{entry.song.name}</span>
                  </Button>
                  ...
                  <Button
                    ...
                    variant="ghost"
                    size="icon-sm"
                    aria-label={`Удалить ${entry.song.name}`}
                    onClick={() =>
                      void run(async () => {
            ...
            {!entries.filter((e) => match(e.song.name)).length && (
              <p className="text-xs text-muted-foreground">
                Нет произведений по этому запросу.
              </p>
            )}
          </div>
          <div className="space-y-2">
            <h3 className="text-sm font-medium">Подборка для игры</h3>
            {catalog
              .filter((s) => match(s.name + " " + s.composer))
              .map((song) => (
                <Button
                  disabled={busy}
                  key={song.id}
          ...
          <p className="text-xs text-muted-foreground">
            Файл .pianopack содержит ноты, настройки и вашу картинку. Отправьте
            его другу, затем импортируйте в этот тренажёр. MIDI подходит для
            других музыкальных программ.
          </p>
          <p className="text-xs text-muted-foreground">
